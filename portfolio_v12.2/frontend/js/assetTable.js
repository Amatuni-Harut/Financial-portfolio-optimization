...
async function recalcBudgetRealtime() {
    ...
    if (total > 0) {
        setBudgetDisplay('$' + total.toLocaleString('en-US', {
        ...
        setBudgetDisplay(allPricesKnown ? '$0.00' : '—', false);
    }
}
...
function updateBudgetDisplay() {
    ...
    if (DashboardState.lastResult?.input_portfolio) {
        ...
        if (budget) {
            setBudgetDisplay('$' + Number(budget).toLocaleString('en-US', {
    ...
    // Иначе — считаем в реальном времени по ценам из API
...
function renderAssetList() {
    ...
    const totalColor  = overLimit  ? 'color:var(--negative);font-weight:700;'
                      ...
                      : allocFilled ? 'color:var(--positive);font-weight:700;' : 'color:var(--text-muted);font-weight:600;';
                ...
                <tr class="tfoot-total-row">
                    ...
                    <td>
                        <div class="total-alloc-cell" style="${totalColor}">
                            ${allocFilled
                                ? `<span style="${totalColor}">${totalAlloc.toFixed(1)}</span><span class="pct-suffix">%</span>`
                                : `<span style="color:var(--text-muted);font-weight:600;">${totalQty > 0 ? totalQty + ' шт.' : '—'}</span>`
                            }
                        </div>
                    </td>
