...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Автосохранение валюты сразу при изменении — без кнопки Save
    els.currency?.addEventListener('change', () => {
        const cur = els.currency.value || 'usd';
        localStorage.setItem('currency', cur);
        AppState.set('currency', cur);
        if (typeof showToast === 'function') showToast('Валюта изменена: ' + cur.toUpperCase(), 'success');
    });
