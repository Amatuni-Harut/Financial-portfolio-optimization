...
# ============================================================
# РЕАЛЬНЫЕ ФИНАНСОВЫЕ МЕТРИКИ
# ============================================================

def _sortino_ratio(returns: np.ndarray, risk_free_monthly: float) -> float:
    """
    Настоящий Sortino Ratio.
    Penalizes только отрицательные отклонения (downside deviation).
    """
...
def _cvar_95(returns: np.ndarray) -> float:
    """
    Настоящий CVaR (Expected Shortfall) на уровне доверия 95%.
    Среднее потерь в худших 5% сценариев. Возвращает % (может быть отрицательным).
    """
    if len(returns) < 10:
        return float(np.min(returns) * 100) if len(returns) else 0.0
    threshold = np.percentile(returns, 5)   # 5-й перцентиль = VaR 95%
    tail = returns[returns <= threshold]
    return float(np.mean(tail) * 100) if len(tail) else float(threshold * 100)


def _diversification_ratio(weights: np.ndarray, cov_matrix: np.ndarray) -> float:
    """
    Настоящий коэффициент диверсификации.
    DR = (weighted avg individual vol) / portfolio vol >= 1.
    DR > 1 означает реальный диверсификационный эффект.
    """
    individual_vols = np.sqrt(np.diag(cov_matrix))
    weighted_avg_vol = float(np.dot(weights, individual_vols))
    portfolio_vol = float(np.sqrt(max(float(np.dot(weights.T, np.dot(cov_matrix, weights))), 0.0)))
    if portfolio_vol <= 0:
        return 1.0
...
def min_cvar_opt(
    ...
    n = len(tickers)
    rng = np.random.default_rng(MC_SEED)   # thread-safe: не мутирует глобальный state

    try:
        scenarios = rng.multivariate_normal(mean_returns, cov_matrix, n_scenarios)
    except Exception:
        scenarios = rng.normal(0, np.sqrt(np.diag(cov_matrix)), (n_scenarios, n))
...
def monte_carlo_opt(
    ...
    n = len(tickers)
    bounds = _build_bounds(tickers, allocation_limits)
    rng = np.random.default_rng(MC_SEED)   # thread-safe
...
def compute_efficient_frontier(
    ...
    rng = np.random.default_rng(0)   # thread-safe
    n = len(mean_returns)
    results = []
...
def analyze_stocks(
    ...
    for ticker in returns_wide.columns:
        ...
        if np.isnan(price) or np.isinf(price):
            price = 0.0
        if len(rets) < 2:
            mr, sr, sharpe = 0.0, 0.0, 0.0
        else:
            mr = float(rets.mean())
            sr = float(rets.std())
            if np.isnan(mr) or np.isinf(mr):
                mr = 0.0
...
def run_optimization(
    ...
    # ── Реальные аналитические метрики лучшего портфеля ──────────
    best_weights = np.array(best["weights"])
    port_ret_series = (returns[available] @ best_weights).values

    real_sortino = _sortino_ratio(port_ret_series, rf_monthly)
    real_cvar_95 = _cvar_95(port_ret_series)
    real_dr      = _diversification_ratio(best_weights, cov_mat)
    ...
    return {
        ...
        "sortino_ratio":      round(real_sortino, 4),
        "cvar_95":            round(real_cvar_95, 4),
        "diversification_ratio": real_dr,
    }
