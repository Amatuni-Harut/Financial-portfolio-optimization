"""
data_service.py — Сервис данных с DB-first логикой. AssetAlpha v14.0
...
Изменения v14:
- SimpleCache: добавлен threading.Lock — потокобезопасность при работе
  из ThreadPoolExecutor (устраняет race condition при конкурентных запросах)
"""
import asyncio
import logging
import threading
...
# ============================================================
# КЕШ — потокобезопасный
# ============================================================

class SimpleCache:
    """
    Thread-safe in-memory кэш с TTL.
    threading.Lock защищает от race condition при конкурентных
    запросах из ThreadPoolExecutor (optimizer workers).
    """
