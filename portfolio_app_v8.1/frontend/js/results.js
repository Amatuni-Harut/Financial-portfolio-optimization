...
function renderProResults(data, container) {
    ...
    // Эффективная граница (новый блок v7)
    if (data.efficient_frontier?.length) {
        html += section('📈 Эффективная граница Марковица', 'NEW',
            canvasBlock('efficientFrontierChart', 360));
    }
    ...
    if (data.efficient_frontier?.length) initEfficientFrontierChart(data);
}
