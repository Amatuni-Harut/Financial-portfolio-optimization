...
const DashboardState = {
    ...
    /** Сохраняет активы и последний результат в localStorage */
    save() {
        try {
            localStorage.setItem('da_assets', JSON.stringify(this.assets));
            localStorage.setItem('da_manual', String(this.isManualWeights));
            if (this.lastResult) {
                // Не сохраняем efficient_frontier — слишком большой
                const slim = { ...this.lastResult };
                delete slim.efficient_frontier;
                localStorage.setItem('da_result', JSON.stringify(slim));
            }
        } catch (e) {
            console.warn('DashboardState.save failed:', e);
        }
    },

    /** Восстанавливает активы и последний результат из localStorage */
    load() {
        try {
            const a = localStorage.getItem('da_assets');
            if (a) this.assets = JSON.parse(a);
            const m = localStorage.getItem('da_manual');
            if (m !== null) this.isManualWeights = (m === 'true');
            const r = localStorage.getItem('da_result');
            if (r) this.lastResult = JSON.parse(r);
        } catch (e) {
            console.warn('DashboardState.load failed:', e);
        }
    },
};
...
document.addEventListener('DOMContentLoaded', () => {
    // Восстанавливаем состояние после навигации (настройки, скринер → обратно)
    DashboardState.load();
    ...
    // Если был сохранён результат — восстанавливаем его
    if (DashboardState.lastResult) {
        const level = AppState.get('knowledgeLevel') || 'beginner';
        try {
            renderResults(DashboardState.lastResult, level);
        } catch(e) {
            console.warn('Не удалось восстановить результаты:', e);
            DashboardState.lastResult = null;
        }
    }

    // Отображаем имя пользователя в sidebar если авторизован
    const username = typeof Auth !== 'undefined' ? Auth.getUsername() : null;
    if (username) {
...
function bindEvents() {
    ...
    // Переключатель периода (День / Месяц / Год)
    document.querySelectorAll('.period-btn').forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            document.getElementById('calcPeriod').value = period;
            // Если есть последний результат — перерендерить с новым периодом
            if (DashboardState.lastResult) {
                const level = AppState.get('knowledgeLevel') || 'beginner';
                renderResults(DashboardState.lastResult, level);
            }
...
async function runOptimize() {
    ...
    try {
        const params = {
            ...
            calcPeriod:        document.getElementById('calcPeriod')?.value  || 'month',
        ...
        DashboardState.save();   // сохраняем для восстановления после навигации
