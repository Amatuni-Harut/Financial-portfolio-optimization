...
function initCompareChart(data, portfolios) {
    ...
    // adaptMetrics доступна из results.js (подключается раньше charts.js)
    const adapt = (m) => (typeof adaptMetrics === 'function' ? adaptMetrics(m) : m);
    const lbl   = (typeof periodLabel === 'function') ? periodLabel() : 'мес';

    // Вспомогательная функция — один датасет из одного значения
    const ds = (getValue) => allPortfolios.map(({ label, p, color }) => ({
        label,
        data: [getValue(adapt(p?.metrics || {}), p?.metrics || {})],
    ...
    // 5. Sharpe Ratio (не меняется от периода)
    safeChart('cmpSharpeChart', {
        type: 'bar',
        data: { labels: [''], datasets: ds((m, raw) => +(raw.sharpe ?? 0).toFixed(4)) },
