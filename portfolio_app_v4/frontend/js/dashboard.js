...
function renderAssetList() {
    ...
    const showQty    = true;          // количество всегда
    const showAlloc  = true;          // Allocation всегда
        ...
        <table class="asset-table">
            ...
            <tbody>
                ${rows || `<tr><td colspan="8" style="text-align:center;padding:24px;color:var(--text-muted);">
                    Добавьте минимум 2 актива через строку поиска
                </td></tr>`}
            </tbody>
            <tfoot>
                <tr>
                    ...
                    <td>
                        <div class="total-alloc-cell">
                            <span class="total-alloc-val">${totalAlloc > 0 ? totalAlloc.toFixed(1) : 0}</span>
...
function openTickerSearch(currentTicker) {
    // Фокусируемся на поле поиска и вставляем текущий тикер для замены
    const searchInput = document.getElementById('searchInput');
    if (searchInput) {
